...
class WebRtcConnection extends EventEmitter {
	constructor(server, socket){
		...
		socket.on('publish', (message) => {
			this.emit('receive', 'publish', message);
		...
		socket.on('request-view', () => {
            let peerConnection = this.getPeerConnection(true,"T");

            this.send(sessionDescription.type, sessionDescription.sdp);
            send('localSDP',sessionDescription.sdp);
			//this.handleAnswer(answer);
		});
        socket.on('view', () => {
            this.emit('view', 'view', answer);
            this.handleAnswer(answer);
        });
	...
	getPeerConnection(usePlanB,ext) {

        let room = this.server.getRoom(this.roomId);
        this.mediaPeer = room.mediaRoom.Peer(this.id+ext);
        ...
        this.peerConnection.on('close', (err) => {
            if(err) {
                this.emit('error', err);
            }
            else {
        ...
        this.peerConnection.on('signalingstatechange', () => {
            this.emit('signaling-state-change', peerConnection.signalingState);
        });
        this.peerConnection.on('leave', () => {
	...
	handleOffer(sdp, usePlanB) {
		...
		this.getPeerConnection(usePlanB,"R");

		// Set the remote SDP offer
        this.peerConnection.setCapabilities(sdp)
		.then(() => {
			this.sendOffer();
		});

		// Handle 'negotiationneeded' event
        this.peerConnection.on('negotiationneeded', () => {
